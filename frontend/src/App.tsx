...
export default function App() {
  const [tab, setTab] = useState<'screener' | 'detail' | 'portfolio' | 'trading' | 'indicators' | 'settings'>('screener');
  ...
  return <div className="app-shell" data-theme={theme}>
    ...
    <nav className="tabs">{([['screener','스크리너'],['detail','종목 상세'],['portfolio','포트폴리오'],['trading','트레이딩'],['indicators','지표 관리'],['settings','설정']] as const).map(([key, label]) => <button className={`tab ${tab === key ? 'active' : ''}`} onClick={() => setTab(key)} key={key}>{label}</button>)}</nav>
