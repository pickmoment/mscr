...
export default function App() {
  const [tab, setTab] = useState<'screener' | 'detail' | 'watchlist' | 'portfolio' | 'trading' | 'indicators' | 'stats' | 'live' | 'settings'>('screener');
  ...
  return <div className="app-shell" data-theme={theme}>
    ...
    <nav className="tabs">{([['screener','스크리너'],['detail','종목 상세'],['watchlist','관심종목'],['stats','시장 통계'],['live','현재 시황'],['portfolio','포트폴리오'],['trading','트레이딩'],['indicators','지표 관리'],['settings','설정']] as const).map(([key, label]) => <button className={`tab ${tab === key ? 'active' : ''}`} onClick={() => setTab(key)} key={key}>{label}</button>)}</nav>
    <main className="content">
      ...
      <section style={{ display: tab === 'detail' ? 'block' : 'none', height: '100%' }}><TickerDetail ticker={selectedTicker} tickers={siblings} onSelect={selectTicker} light={theme === 'light'} /></section>
      <section style={{ display: tab === 'watchlist' ? 'block' : 'none', height: '100%' }}><WatchlistPanel onSelect={selectTicker} /></section>
