...
import { api, LiveFeaturedSection, LiveIssueRow, LiveMarketStat, LiveNewsRow, LiveThemeRow, LiveThemeStock, LiveTrendingRow, MarketLiveOverview } from '../lib/api';
...
function FeaturedTable({ factor, section, onSelect }: { factor: string; section: LiveFeaturedSection; onSelect: (ticker: string) => void }) {
  return <div className="panel" style={{ padding: 14, minWidth: 0 }}>
    <div className="section-title" style={{ margin: '0 0 10px' }}>{section.label}{section.error && <span className="subtle"> · 조회 실패</span>}</div>
    <div className="rank-scroll">
      <table className="metric-table rank-table">
        <thead><tr><td>종목</td><td>종가</td><td>등락률</td><td>거래대금</td></tr></thead>
        <tbody>
          {section.rows.map(row => <tr key={`${factor}-${row.code}`} className="rank-row" onClick={() => onSelect(row.code)}>
            ...
            <td className={`mono ${changeClass(row.returns)}`}>{fmtPct2(row.returns)}</td>
            <td className="mono">{compactVolume(row.volume_valued)}원</td>
          </tr>)}
  ...
  return <div className="scroll market-stats" style={{ padding: 18, height: '100%' }}>
    ...
    {overview && <>
      ...
      <div className="card-grid" style={{ gridTemplateColumns: 'repeat(3,1fr)', alignItems: 'start' }}>
        {Object.entries(overview.featured).map(([factor, section]) => <FeaturedTable key={factor} factor={factor} section={section} onSelect={onSelect} />)}
      </div>
