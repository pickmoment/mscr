...
export default function ScreenerPanel({ onResults, onPresetChange, onSortFormulaChange }: {
  onResults: (rows: ScreenRow[]) => void;
  onPresetChange: (preset: { id: number; name: string } | null) => void;
  onSortFormulaChange: (formula: string) => void;
}) {
  ...
  // 선택·저장·갱신·삭제가 저마다 부모에 알리면 한 곳만 빠져도 오른쪽 탭이 어긋난다. 결과 상태 하나만 보고 올린다.
  useEffect(() => onPresetChange(selectedPreset ? { id: selectedPreset.id, name: selectedPreset.name } : null), [selectedPreset?.id, selectedPreset?.name]);
  // 결과 표의 '정렬 수식값' 열이 무엇을 계산한 값인지 보여주려면 지금 수식이 필요하다.
  useEffect(() => onSortFormulaChange(spec.sort.formula), [spec.sort.formula]);
  const applyPreset = () => {
  ...
  return <aside className="panel scroll screener-panel stack">
    ...
    <div className="toolbar screener-run">
